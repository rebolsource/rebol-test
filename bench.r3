...
time-tick: func [
{
^-^-Time the system clock tick for a computer.
^-^-Works reliably, if the execution time 
^-^-of the loop body is shorter than the clock tick time.
^-^-Otherwise it computes the loop body execution time
^-^-instead of the system clock tick time.
^-}
precision [decimal!] "suggested value: 0.05 to 0.30"
/local guess count start time result prev cur ticks
] [
guess: 0
count: 1
while [
...
result: (to decimal! difference cur start) / (ticks + 0.5)
any [
ticks <= 0 (abs result - guess) / result + (1.5 / ticks) > precision
]
] [
guess: result
count: count * 2
...
if ticks + 1 >= count [
print {Warning: clock tick too short, the result is the loop body execution time!}
]
result
]
time-block: func [
...
any [
result <= 0 (abs result - guess) / result + (3 * tick-time / time) > precision
]
] [
guess: result
count: count * 2
...
^-Clock ticks for some systems:
^-(let me know about your results)

^-MS Windows 95/98^-^-55 milliseconds
^-MS Windows NT 4.0^-^-10 milliseconds
^-MS Windows XP^-^-^-10 milliseconds
^-MS Windows XP SP2^-^-15.5 milliseconds
^-MS Windows XP SP3^-^-15.5 milliseconds
^-MS Win Vista 32^-^-^-1 millisecond
^-MS Win Vista Bus. 64^-15.5 milliseconds
^-MS Windows 7^-^-^-1 millisecond
^-
^-Linux^-^-^-^-^-better than 25 microseconds
^-Mac OS X 10^-^-^-^-better than 2 microseconds^-^-^-^-
}
tick-time: time-tick 0.05
sieve: function [size] [flags i prime series] [
...
] [h m sum alpha beta sqrt3 halfh] [
h: (b - a) / n
halfh: h / 2
m: 0
sum: 0
